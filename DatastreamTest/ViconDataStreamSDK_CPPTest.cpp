...
#ifdef WIN32
  // If any key is pressed the application stops 
  bool Hit()
  {
    bool hit = false;
    while( _kbhit() )
...
int main( int argc, char* argv[] )
{
  ...
  for(int i=0; i != 3; ++i) // repeat to check disconnecting doesn't wreck next connect
  {
	...
	std::ofstream subjectDataFile;
	std::string subjectFileName = "SubjectData.csv";
	std::ofstream unlabelledPointsFile;
	std::string unlabelledFileName = "UnlabelledData.csv";
  ...
  // Loop until a key is pressed
  ...
  #endif
    {
      ...
      if(++Counter == FrameRateWindow)
      {
        ...
        if(!LogFile.empty())
        {
          ...
          ofs << "Frame rate = " << FrameRate << " at " <<  asctime_s (&temp, 100, &timeinfo)<< std::endl; 
        }
	  ...
	  // Create the file for first time and then no need. However one subject must be present in the stream to create this file. 

	  // If file does not exist create one 
	  bool result = std::experimental::filesystem::exists(subjectFileName);
	  if(!result) {
		  subjectDataFile.open(subjectFileName);
		  subjectDataFile << " Frame No ; Subject Name ; Rx ; Ry ; Rz ; Tx ; Ty ; Tz \n";
	  }
	  // if file is not opne, open it 
	  if(!subjectDataFile.is_open()){
		  subjectDataFile.open(subjectFileName, std::ios_base::app);
	  }

	  // If file does not exist create one 
	  bool fileExists = std::experimental::filesystem::exists(unlabelledFileName);
	  if (!fileExists) {
		  unlabelledPointsFile.open(unlabelledFileName);
		  unlabelledPointsFile << " Frame No ; Point ID ; Px ; Py ; Pz \n";
	  }
	  // if file is not open we open it and append the data  
	  if (!unlabelledPointsFile.is_open()) {
		  unlabelledPointsFile.open(unlabelledFileName, std::ios_base::app);
	  }
      ...
      for( unsigned int SubjectIndex = 0 ; SubjectIndex < SubjectCount ; ++SubjectIndex )
      {
		...
		// 
		// If my file exists we put in the frame count 
		if (subjectDataFile.is_open()) {
			subjectDataFile << _Output_GetHardwareFrameNumber.HardwareFrameNumber;
			subjectDataFile << ";" << SubjectName;
		}
        ...
        for( unsigned int SegmentIndex = 0 ; SegmentIndex < SegmentCount ; ++SegmentIndex )
        {
		  ...
		  // Add rotation and translation data to the file. 
		  if (subjectDataFile.is_open()) {
			  // Save rotation values 
			  subjectDataFile << ";" << _Output_GetSegmentGlobalRotationEulerXYZ.Rotation[0];
			  subjectDataFile << ";" << _Output_GetSegmentGlobalRotationEulerXYZ.Rotation[1];
			  subjectDataFile << ";" << _Output_GetSegmentGlobalRotationEulerXYZ.Rotation[2];

			  // Save translation values 
			  subjectDataFile << ";" << _Output_GetSegmentGlobalTranslation.Translation[0] << "; "
				  << _Output_GetSegmentGlobalTranslation.Translation[1] << "; "
				  << _Output_GetSegmentGlobalTranslation.Translation[2] << "\n" ;
		  }
      ...
      { 
		...
		if(unlabelledPointsFile.is_open()){
			unlabelledPointsFile << _Output_GetFrameNumber.FrameNumber << ";" ;
			unlabelledPointsFile << UnlabeledMarkerIndex << ";";
			unlabelledPointsFile << _Output_GetUnlabeledMarkerGlobalTranslation.Translation[0] << ";" ;
			unlabelledPointsFile << _Output_GetUnlabeledMarkerGlobalTranslation.Translation[1] << ";" ;
			unlabelledPointsFile << _Output_GetUnlabeledMarkerGlobalTranslation.Translation[2] << "\n" ;
		}
    ...
    } // End - While loop reading the stream data 

	if (subjectDataFile.is_open()) {
		subjectDataFile.close();
	}

	if (unlabelledPointsFile.is_open()) {
		unlabelledPointsFile.close();
	}
  ...
  } // Ending for loop connecting with the server, 3 connection attempts are made. 
}
